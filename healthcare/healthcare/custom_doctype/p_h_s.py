...
from healthcare.healthcare.custom_doctype.patient_history import get_patient_history_doctypes
...
def create_medical_record(doc, method=None):
	print('uaYSFDYUTFTFGTGFFGYTF87TTFGHFTYFGT	')
	medical_record_required = validate_medical_record_required(doc)
	print(medical_record_required,'medical_record_requiredmedical_record_required')
	if not medical_record_required:
		return

	reference = doc.name
	print(reference,'referencereference')
...
def update_medical_record(doc, method=None, reference=None):
	print("kkkkkkshfhdgfhdsfgjkhdfagjkfgsdjcsfdgv")
	medical_record_required = validate_medical_record_required(doc)
	if not medical_record_required:
		return
